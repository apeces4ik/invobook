    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <select id="status-select" class="rounded-md border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <a href="{{ route('admin.tickets.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-blue-600 transition px-3 py-1 border border-gray-300 rounded-md">Назад</a>
            </div>
        </div>
        <div class="px-6 py-6">
            <div class="mb-6 bg-gray-50 rounded-lg p-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p><span class="font-semibold">Пользователь:</span> {{ $ticket->user->name }}</p>
                    </div>
                    <div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function sendMessage(content) {
        try {
            const response = await fetch(`/admin/tickets/${ticketId}/messages`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('status-select').addEventListener('change', function() {
        ...
        .then(data => {
            if (data.success) {
                // Можно добавить уведомление об успешном изменении статуса
            }
        })
        .catch(error => {
